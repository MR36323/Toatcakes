...
def create_fact_sales_order(
    sales_order: list, previous_df: pd.DataFrame
) -> pd.DataFrame:
    ...
    new_df["last_updated_time"] = [
        date_time.split(" ")[1] for date_time in last_updated_list
    ]
...
def get_latest_transformed_object_from_S3():
    ...
    objects_response = s3_client.list_objects_v2(
        Bucket=os.environ.get("PROCCESSED_BUCKET"), Prefix="fact_sales_order"
    )
    ...
    current_data = s3_client.get_object(
        Bucket=os.environ.get("PROCCESSED_BUCKET"), Key=most_recent_key
    )["Body"].read()

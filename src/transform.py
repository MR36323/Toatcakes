...
def lambda_handler(event,context):
    ...
    input_data = {table_name: get_data(table_name, os.environ.get('INGESTION_BUCKET')) for table_name in tables}
    dim_staff_df = create_dim_staff(input_data['staff'], input_data['department'])
    dim_counterparty_df = create_dim_counterparty(input_data['counterparty'], input_data['address'])
    ...
    dataframes = [dim_staff_df,dim_counterparty_df,dim_currency_df,dim_design_df,dim_location_df,dim_date_df,fact_sales_df]
    s3_client = client('s3')
    try:
        responses = [reformat(df,os.environ.get('PROCESSED_BUCKET'), s3_client) for df in dataframes]

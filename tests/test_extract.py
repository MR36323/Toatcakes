...
def test_make_and_close_connection_are_called_once(mock_make_connection, mock_close_connection, mock_check_data_updates):
...
@patch("src.extract.make_connection")
def test_data_gets_called_once_per_table(mock_make_connection, mock_get_data, mock_check_data_updates):
    mock_conn = Mock()
...
@patch("src.extract.get_data")
@patch("src.extract.make_connection")
def test_format_of_input_data_is_correct(mock_make_connection, mock_get_data, mock_s3_client, mock_check_data_updates, mock_data_to_bucket):
    mock_conn = Mock()
    mock_client = Mock()
    mock_make_connection.return_value = mock_conn
    ...
    def fake_get_data(conn, query, table_name):
        fake_data = {'staff': [{"staff_first_name": "matthew", "staff_last_name": "burns"}, {"staff_first_name": "beth", "staff_last_name": "suffield"}]}
        return fake_data
...
@patch("src.extract.get_data")
@patch("src.extract.make_connection")
def test_data_to_bucket_is_called_with_correct_arguments(mock_make_connection, mock_get_data, mock_s3_client, mock_check_data_updates, mock_data_to_bucket):
    mock_conn = Mock()
    mock_client = Mock()
    mock_make_connection.return_value = mock_conn
    mock_s3_client.return_value = mock_client
...
@patch("src.extract.get_data")
@patch("src.extract.make_connection")
def test_s3_client_is_created(mock_make_connection, mock_get_data, mock_check_data_updates, mock_s3_client):
    mock_conn = Mock()
    mock_make_connection.return_value = mock_conn
    mock_check_data_updates.return_value = False
